function pop (e) {
    let txt_box = e.target.parentNode.parentNode.querySelector(".box")
    let parent = e.target.parentNode.parentNode;
    let img = parent.querySelector(".mobile_item");
    let style = img.currentStyle || window.getComputedStyle(img, false);
    // let bg_img = style.backgroundImage.slice(4, -1).replace(/"/g, "");
    let bg_img = "icons/favicon_/favicon-32x32.png"
    let amount = 30;
    ...
    if (e.clientX === 0 && e.clientY === 0) {
        ...
        for (let i = 0; i < 30; i++) {
            createParticle(x, y, e.target.dataset.type, bg_img);
        }
        } else {
        for (let i = 0; i < amount; i++) {
            createParticle(e.clientX, e.clientY, e.target.dataset.type, bg_img);
        }
    }
    img.classList.add("opacity-0")
    txt_box.classList.add("box_vissible")
    txt_box.querySelector(".box_btn-right").addEventListener("click", (e) => {
        img.classList.remove("opacity-0")
        txt_box.classList.remove("box_vissible")
    })
}
function createParticle (x, y, type, img) {
    ...
    switch (type) {
        ...
        break;
        case 'logo':
        particle.style.background = `url(${img}) center center/cover no-repeat, rgb(241 243 244 / 20%)`; // Ссылка на картинку
        break;
        case 'shadow':
...
document.querySelectorAll('.mobile_item_plus').forEach(plus => plus.addEventListener('click', pop));
